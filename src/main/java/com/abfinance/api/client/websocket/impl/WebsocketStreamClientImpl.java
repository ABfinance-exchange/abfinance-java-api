...
import com.abfinance.api.client.websocket.httpclient.WebSocketStreamHttpClientSingleton;
...
import com.abfinance.api.client.security.HmacSHA256Signer;
...
import static com.abfinance.api.client.constant.Helper.generateTimestamp;
...
public class WebsocketStreamClientImpl implements WebsocketStreamClient {
    ...
    private List<Map<String, Object>> constructWsAPIArgs(Map<String,Object> originalParams) {
        ...
        params.remove(ABFinanceApiConstants.TIMESTAMP_HEADER);
        params.remove("reqId");
        params.remove(ABFinanceApiConstants.RECV_WINDOW_HEADER);
        params.remove(ABFinanceApiConstants.BROKER_HEADER);
        return Collections.singletonList(params);
    }

    private Map<String, String> constructWsAPIHeaders(Map<String,Object> params) {
        Map<String,String> headerMap = new HashMap<>();
        headerMap.put(ABFinanceApiConstants.TIMESTAMP_HEADER, String.valueOf(generateTimestamp()));
        headerMap.put(ABFinanceApiConstants.RECV_WINDOW_HEADER, params.getOrDefault(ABFinanceApiConstants.RECV_WINDOW_HEADER, ABFinanceApiConstants.DEFAULT_RECEIVING_WINDOW).toString());
        // Check broker referral code
        if(params.containsKey(ABFinanceApiConstants.BROKER_HEADER))
            headerMap.put(ABFinanceApiConstants.BROKER_HEADER, params.get(ABFinanceApiConstants.BROKER_HEADER).toString());
        return headerMap;
    }

    private boolean requiresAuthentication(String path) {
        return ABFinanceApiConfig.V5_TRADE.equals(path) ||
                ABFinanceApiConfig.V5_PRIVATE.equals(path);
/*                ABFinanceApiConfig.V3_CONTRACT_PRIVATE.equals(path) ||
                ABFinanceApiConfig.V3_UNIFIED_PRIVATE.equals(path) ||
                ABFinanceApiConfig.V3_SPOT_PRIVATE.equals(path);*/
    }
    ...
    public void onOpen(WebSocket ws) {
        ...
        if (requiresAuthentication(path)) {
            Thread authThread = createAuthThread(ws, () -> {
                ...
                if(path.equals(ABFinanceApiConfig.V5_TRADE)){
